...
#define arm_radius_back_checksum         CHECKSUM("arm_radius_back")
...
ExperimentalDeltaSolution::ExperimentalDeltaSolution(Config* config)
{
    ...
    arm_radius_back = config->value(arm_radius_back_checksum)->by_default(124.0f)->as_number();
    init();
}

void ExperimentalDeltaSolution::init() {
    arm_length_squared = SQ(arm_length);

     // Effective X/Y positions of the three vertical towers.
    float delta_radius = arm_radius;

    delta_tower1_x = -arm_radius; // front left tower
    delta_tower1_y = 0.0F;
    delta_tower2_x = arm_radius; // front right tower
    delta_tower2_y = 0.0F;
    delta_tower3_x = 0.0F; // back middle tower
    delta_tower3_y = arm_radius_back;
}
...
bool ExperimentalDeltaSolution::set_optional(const arm_options_t& options) {

    for(auto &i : options) {
        switch(i.first) {
            case 'L': arm_length= i.second; break;
            case 'R': arm_radius= i.second; break;
            case 'B': arm_radius_back= i.second; break;
        }
    }
    init();
...
bool ExperimentalDeltaSolution::get_optional(arm_options_t& options, bool force_all) {
    options['L']= this->arm_length;
    options['R']= this->arm_radius;
    options['B']= this->arm_radius_back;
